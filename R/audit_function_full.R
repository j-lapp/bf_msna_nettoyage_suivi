...
time_check <- function(df, time_limit_low, time_limit_high){
  df <- df %>% mutate(start = ymd_hms(start), end = ymd_hms(end),
                      duration_entretien = difftime(as.POSIXct(end), as.POSIXct(start), units = "mins"),
                      time_validity = case_when( 
                        duration_entretien < time_limit_low ~ "trop_court",
                        duration_entretien >= time_limit_low && duration_entretien <= time_limit_high  ~ "Valide",
                        duration_entretien > time_limit_high ~ "trop_court",
